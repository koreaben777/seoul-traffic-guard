...
2. Tool은 MVP 범위인 8개로 유지하고, 새 tool 추가는 심사 전까지 금지한다.
...
| `preview_alert_message` | 아니오 | 발송 없음 |
...
| `preview_alert_message` | 발송 전 메시지 미리보기 | readOnlyHint=true |
...
MVP에서는 8개 툴만 만든다. `set_commute_window`, 경로 최적화, 친구에게 보내기는 제외한다. 카카오톡 메시지는 KakaoTalk Message API의 "나에게 보내기" 범위로 제한한다. 공식 문서 기준 기본 템플릿 나에게 보내기는 `talk_message` 동의항목과 액세스 토큰이 필요하며, 친구 등 다른 사용자에게 보내기는 별도 사용 권한이 필요하므로 MVP에서 제외한다.

예약 알림은 사용자가 등록한 생활권 label을 기준으로 동작한다. 예를 들어 `출근길` 생활권을 등록한 뒤 `set_scheduled_alert`로 평일 07:00 예약을 만들면, 서버는 해당 시각에 교통 이슈를 확인하고 본인 카카오톡으로 알림을 보낸다. MVP에서는 공휴일 캘린더, 임시 정지 기간, 복잡한 반복 규칙은 제외한다.
...
- MCP Inspector에서 8개 tool이 모두 보인다.
...
| 알림 미리보기 | `이 내용을 알림 문구로 만들어줘.` | `preview_alert_message` |
| 즉시 발송 | `카카오톡으로 나에게 보내줘.` | `send_self_alert` |
...
- 로컬에서는 `python3 scripts/local_beta_flow.py`로 등록, 조회, 이슈 확인, 알림 미리보기 흐름을 한 번에 점검한다.
...
| 06-22 | 로컬 MCP MVP | 8개 툴, SQLite 저장소, OAuth 나에게 보내기, Docker 빌드 |
